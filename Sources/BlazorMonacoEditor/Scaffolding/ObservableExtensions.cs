...
internal static class ObservableExtensions
{
    ...
    {
        return observable.Select(x => Observable.FromAsync((token) => supplier(x, token), new TaskObservationOptions(scheduler: null, ignoreExceptionsAfterUnsubscribe: true)).Take(1)).Concat();
    }
    ...
    {
        return observable.Select(x => Observable.FromAsync(token => supplier(x, token), new TaskObservationOptions(scheduler: null, ignoreExceptionsAfterUnsubscribe: true)).Take(1)).Concat().Subscribe(_ => { }, onError);
    }
    
    public static IDisposable SubscribeAsync<T>(this IObservable<T> observable, Func<T, Task> supplier, Action<Exception> onError)
    {
        return observable.Select(x => Observable.FromAsync(_ => supplier(x), new TaskObservationOptions(scheduler: null, ignoreExceptionsAfterUnsubscribe: true)).Take(1)).Concat().Subscribe(_ => { }, onError);
    }
